...
#import "TtougaoViewController.h"
...
#define MARIGN 10
#define FONTSIZE 12
@interface TuserViewController ()

@end

@implementation TuserViewController
    ...
    [NSURLConnection sendAsynchronousRequest:urlRequest queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        else
        {
            ...
            if (error) {
                ...
                self.tuser = [[TuserModel alloc] init];
                self.tuser.userName = [[tempDic objectForKey:@"data"] objectForKey:@"name"];
                //self.tuser.description = [[tempDic objectForKey:@"data"] objectForKey:@"description"];
                self.tuser.point = [[tempDic objectForKey:@"data"] objectForKey:@"point"];
                self.tuser.ugc = [[tempDic objectForKey:@"data"] objectForKey:@"ugc_count"];
                self.tuser.userPicture = [[tempDic objectForKey:@"data"] objectForKey:@"avatar_url"];
                self.tuser.essay = [[tempDic objectForKey:@"data"] objectForKey:@"essay_repin_count"];
                self.tuser.comment = [[tempDic objectForKey:@"data"] objectForKey:@"comment_count"];
                
                NSLog(@"%@", [[tempDic objectForKey:@"data"] objectForKey:@"name"] );
                
                
                [self createView:_tuser];
                [self createMessage:_tuser];
            }
...
- (void)createView:(TuserModel *)user
{
    UIView *userView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, 320, 100)];
    ...
    imagePicture.layer.cornerRadius = 3.0;
...
- (void)createMessage:(TuserModel *)user
{
    UIImageView *aImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 120 + 64, 320, 30)];
...
- (void)tougao
{
    TtougaoViewController *tougaoVC = [[TtougaoViewController alloc] init];
...
- (void)shoucang
{
    TshoucangViewController *shoucangVC = [[TshoucangViewController alloc] init];
